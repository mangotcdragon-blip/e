...
/**
 * The shared doomscroll viewer used by both the main browse feed and the Liked page, so both get
 * identical zoom/playback/caching/like/share/download behavior from one implementation.
 */
...
fun MediaViewerScreen(
    posts: List<Post>,
    initialIndex: Int,
    externalCacheEnabled: Boolean,
    externalCacheTreeUri: String?,
    isLiked: (Post) -> Boolean,
    onToggleLike: (Post) -> Unit,
    onBack: () -> Unit,
    onLaunchingExternalActivity: () -> Unit,
    onPageSettled: (Post) -> Unit = {},
    onNearEnd: () -> Unit = {},
) {
    ...
    LaunchedEffect(pagerState.currentPage, posts.size) {
        ...
        if (pagerState.currentPage >= posts.size - 5) {
            onNearEnd()
        }
    }
        ...
        topBar = {
            TopAppBar(
                ...
                actions = {
                    ...
                    IconButton(onClick = { current?.let(onToggleLike) }) {
                        val liked = current != null && isLiked(current)
                        Icon(
                            imageVector = if (liked) Icons.Filled.Favorite else Icons.Filled.FavoriteBorder,
                            contentDescription = if (liked) "Unlike" else "Like",
                            tint = if (liked) Color(0xFFFF5C8A) else Color.White,
                        )
                    }
                    IconButton(onClick = {
                        current?.let {
...
private fun rememberResolvedMediaUrl(
    ...
    externalCacheTreeUri: String?,
    post: Post,
) = produceState(initialValue = post.viewUrl, post.id, externalCacheEnabled, externalCacheTreeUri) {
    if (!externalCacheEnabled) {
        value = post.viewUrl
        return@produceState
    }
    val cachedFile = ExternalCache.cachedDecryptedFile(context, externalCacheTreeUri, post)
    if (cachedFile != null) {
        value = Uri.fromFile(cachedFile).toString()
    } else {
        // Only worth touching the drive again if we didn't already have a usable copy.
        ExternalCache.cacheInBackground(context, externalCacheTreeUri, post, post.viewUrl)
    }
}
